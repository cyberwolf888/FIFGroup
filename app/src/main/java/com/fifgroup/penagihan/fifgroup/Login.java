    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                jsonStr = webreq.sendPostRequest("login.php", data);

                try {
                    JSONObject jObj = new JSONObject(jsonStr);
                    String status =  jObj.getString("status");
                    if(status.equals("1")){
                        String nama_kolektor = jObj.getString("nama_kolektor");
                        String id_kolektor = jObj.getString("id_kolektor");

                        SharedPreferences.Editor editor = sharedpreferences.edit();
                        editor.putString(Name, nama_kolektor);
                        editor.putString(ID, id_kolektor);
                        editor.commit();
                        return true;
                    }else{
                        return false;
